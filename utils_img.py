...
class SSIMAttenuation:
    ...
    def __init__(self, window_size=17, sigma=1.5, device="cpu"):
        ...
        self.window_size = window_size  # 高斯窗的大小
        # 创建一维的高斯窗口
        ...
        _1D_window = (_1D_window / _1D_window.sum()).unsqueeze(1)  # 归一化
        # 创建二维的高斯窗口
        # 一维高斯窗乘上其转置得到二维高斯窗
        _2D_window = _1D_window.mm(_1D_window.t()).float().unsqueeze(0).unsqueeze(0)
        # 包装成一个可用于梯度计算的变量，确保在训练过程中可以进行反向传播
        self.window = Variable(
            _2D_window.expand(3, 1, window_size, window_size).contiguous()
        )  # 在内存中以连续的方式存储数据
    ...
    def heatmap(self, img1, img2):
        ...
        # padding保证输入输出图片大小相同，groups在三个通道上独立进行卷积
        mu1 = F.conv2d(img1, window, padding=window_size // 2, groups=3)
        mu2 = F.conv2d(img2, window, padding=window_size // 2, groups=3)

        mu1_sq = mu1.pow(2)
        mu2_sq = mu2.pow(2)
        mu1_mu2 = mu1 * mu2  # 逐元素相乘

        sigma1_sq = (
            F.conv2d(img1 * img1, window, padding=window_size // 2, groups=3) - mu1_sq
        )
        sigma2_sq = (
            F.conv2d(img2 * img2, window, padding=window_size // 2, groups=3) - mu2_sq
        )
        sigma12 = (
            F.conv2d(img1 * img2, window, padding=window_size // 2, groups=3) - mu1_mu2
        )
    ...
    def apply(self, x, y):
        ...
        # 在通道维度（第1维度）上进行求和，结果保留维度
        ssim_map = torch.sum(ssim_map, dim=1, keepdim=True)  # 1x1xHxW
        ssim_map = torch.clamp_min(ssim_map, 0)  # 设置最小值为0
        delta = delta * ssim_map  # 高相似度变化小，低相似度变化大
        return y + delta  # 返回增强后的图像
...
def get_dataloader(
    data_dir, transform=default_transform, batch_size=128, shuffle=False, num_workers=4, resize_to_fit=False
):
    """
    ...
    shuffle:混洗
    num_workers:子进程数
    resize_to_fit:是否将图像resize到224x224以适应模型输入 (Default: False)
    """
    if resize_to_fit:
